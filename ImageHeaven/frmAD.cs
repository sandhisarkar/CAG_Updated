...
namespace ImageHeaven
{
    public partial class frmAD : Form
    {
        ...
        public frmAD(string proj, string bundle, OdbcConnection pCon, Credentials pcrd, DataLayerDefs.Mode mode, eSTATES[] prmState)
        {
            InitializeComponent();
        ...
        public frmAD(string proj, string bundle, OdbcConnection pCon, Credentials pcrd, DataLayerDefs.Mode mode, string file, string stage)
        {
            InitializeComponent();
        ...
        public frmAD(string proj, string bundle, OdbcConnection pCon, Credentials pcrd, DataLayerDefs.Mode mode, string file)
        {
            InitializeComponent();
        ...
        private void frmAD_Load(object sender, EventArgs e)
        {
            ...
            if (_mode == Mode._Edit)
            {
                ...
                string id_no = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][8].ToString();
                string desg = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][9].ToString();
                string branch = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][10].ToString();
                string sub_doc_type = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][11].ToString();
                string dob = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][12].ToString();
                string doj = _GetFileCaseDetailsIndividual(projKey, bundleKey, old_filename).Rows[0][13].ToString();
                ...
                deTextBox15.Text = id_no;
                deTextBox16.Text = desg;
                deTextBox7.Text = branch;
        ...
        {
            DataTable dt = new DataTable();
            string sql = "select distinct proj_code, batch_Key,item_no,filename,department,subcat,state_name,emp_name,id_no,desg,branch_name,sub_doc_type,date_format(birth_date,'%Y-%m-%d'),date_format(joining_date,'%Y-%m-%d')," +
                "date_format(retirement_date,'%Y-%m-%d') " +
        ...
        private void frmAD_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        public bool validate()
        {
            ...
            if (deTextBox15.Text == "" || deTextBox15.Text == null || String.IsNullOrEmpty(deTextBox15.Text) || String.IsNullOrWhiteSpace(deTextBox15.Text))
            {
                ...
                MessageBox.Show("You cannot leave ID number field blank...", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
                deTextBox15.Focus();
                return retval;
            }
        ...
        private bool insertIntoMeta(int itemno, string dept, string subCat, string state_name, string filename, string emp_name, string id_no, string desg, string branch, string sub_doc, string bdate, string jdate, string rdate, OdbcTransaction trans)
        {
            bool commitBol = true;
            ...
            if (frmAD.state[0] == eSTATES.METADATA_ENTRY)
            {
                sqlStr = @"insert into metadata_entry(proj_code,batch_key,item_no,department,subcat,state_name,filename,emp_name,id_no,desg,branch_name,sub_doc_type,birth_date,joining_date,retirement_date,created_by,created_dttm,status) values('" +
                        projKey + "','" + bundleKey + "','" + itemno +
                        "','" + dept + "','" + subCat + "','" + state_name + "','" + filename + "','" + emp_name + "','" + id_no + "','" + desg + "','" + branch + "','" + sub_doc + "','" + bdate + "','" + jdate + "','" + rdate + "','" + crd.created_by + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',0)";
            ...
            else
            {
                sqlStr = @"insert into metadata_entry(proj_code,batch_key,item_no,department,subcat,state_name,filename,emp_name,id_no,desg,branch_name,sub_doc_type,birth_date,joining_date,retirement_date,created_by,created_dttm,status) values('" +
                        projKey + "','" + bundleKey + "','" + itemno +
                        "','" + dept + "','" + subCat + "','" + state_name + "','" + filename + "','" + emp_name + "','" + id_no + "','" + desg + "','" + branch + "','" + sub_doc + "','" + bdate + "','" + jdate + "','" + rdate + "','" + crd.created_by + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',5)";
                //sqlCmd.Connection = sqlCon;
            }
            sqlCmd.Connection = sqlCon;
        ...
        public bool updateMetaEdit(string dept, string subCat, string state_name, string emp_name, string id, string des, string branch, string subdoc, string bdate, string jdate, string rdate)
        {
            bool ret = false;
            if (ret == false)
            {
                _UpdateMetaEdit(projKey, bundleKey, old_filename, filename, dept, subCat, state_name, emp_name, id, des, branch, subdoc, bdate,
                    jdate,  rdate);
        ...
        public bool _UpdateMetaEdit(string projKey, string bundleKey, string oldFileName, string fileName,
            string dept, string subCat, string state_name, string emp_name, string id, string des, string branch, string subdoc, string bdate,
            string jdate, string rdate)
        {
            string sqlStr = null;
            ...
            sqlStr = "UPDATE metadata_entry SET filename = '" + fileName + "',department  = '" + dept + "',subcat = '" + subCat + "'," +
                "state_name = '" + state_name + "',emp_name = '" + emp_name + "',id_no= '" + id + "',desg='" + des + "',branch_name='" + branch + "',sub_doc_type='" + subdoc + "'," +
                "birth_date='" + bdate + "',joining_date='" + jdate + "',retirement_date='" + rdate + "'," +
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    if (validate() == true)
                    {
                        ...
                        if (checkFileNotExists(filename, dept, subCat) == true)
                        {
                            ...
                            string id_no = deTextBox15.Text.Trim();
                            string desg = deTextBox16.Text.Trim();
                            string branch = deTextBox7.Text.Trim();
                            ...
                            bool insertCase = insertIntoMeta(item, dept, subCat, statename, filename, emp_name, id_no, desg, branch, sub_doc_type, bDate, joinDate, rDate, sqlTrans);
                            if (insertCase == true)
                            {
                                if (sqlTrans == null)
                                {
                    ...
                    if (validate() == true)
                    {
                        ...
                        if (deTextBox15.Text.Trim() != Files.filename)
                        {
                            if (checkFileNotExistsEdit(deTextBox15.Text.Trim(), dept, subCat) == true)
                            {
                                ...
                                bool updateMeta = updateMetaEdit(dept, subCat, statename, emp_name, ID_no, desg, branch, sub_doc_type, bDate, joinDate,
                                 rDate);
