...
namespace ImageHeaven
{
    public partial class frmNomination : Form
    {
        ...
        public frmNomination(string proj, string bundle, OdbcConnection pCon, Credentials pcrd, DataLayerDefs.Mode mode, eSTATES[] prmState)
        {
            InitializeComponent();
        ...
        public frmNomination(string proj, string bundle, OdbcConnection pCon, Credentials pcrd, DataLayerDefs.Mode mode, string file, string stage)
        {
            InitializeComponent();
        ...
        public DataTable getNominationList(string proj, string bundle)
        {
            DataTable dt = new DataTable();
            string sql = "select distinct proj_code, batch_Key,item_no,filename,department,subcat,state_name,emp_name,desg,fileid,date_format(birth_date,'%Y-%m-%d'),date_format(joining_date,'%Y-%m-%d'),date_format(death_date,'%Y-%m-%d')," +
                "date_format(retirement_date,'%Y-%m-%d'),psa_name,section,pension_file_no,ppo_fppo,gpo_dgpo,ppo_gpo_cpo,mobile,hrms_id,spouce,place_payment,rule_file,vol,subject,series,acc,subscriber_name," +
        ...
        private void frmNomination_Load(object sender, EventArgs e)
        {
            if (_mode == Mode._Add)
            {
                ...
                if(getNominationList(projKey,bundleKey).Rows.Count>0)
                {
                    string sec = getNominationList(projKey, bundleKey).Rows[getNominationList(projKey, bundleKey).Rows.Count-1][15].ToString();
                    string series = getNominationList(projKey, bundleKey).Rows[getNominationList(projKey, bundleKey).Rows.Count-1][27].ToString();
                    deTextBox7.Text = sec;
        ...
        private bool checkFileNotExistsEdit(string file, string dept, string subCat)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                MessageBox.Show("This nomination file is already exists for batch - " + dt1.Rows[0][0].ToString(), "B'Zer - Confirmation !", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                retval = false;
                deTextBox8.Focus();
                return retval;
            }
        ...
        private bool insertIntoMeta(int itemno, string dept, string subCat, string state_name, string filename, string section, string series, string acc, string subsc_name, string bdate, string adate, string hrms, string mobile, OdbcTransaction trans)
        {
            bool commitBol = true;
            ...
            if (frmNomination.state[0] == eSTATES.METADATA_ENTRY)
            {
                sqlStr = @"insert into metadata_entry(proj_code,batch_key,item_no,department,subcat,state_name,filename,section,series,acc,subscriber_name,birth_date,accept_date,hrms_id,mobile,created_by,created_dttm,status) values('" +
                        projKey + "','" + bundleKey + "','" + itemno +
                        "','" + dept + "','" + subCat + "','" + state_name + "','" + filename + "','" + section + "','" + series + "','" + acc + "','" + subsc_name + "','" + bdate + "','" + adate + "','" + hrms + "','" + mobile + "','" + crd.created_by + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',0)";
            ...
            else
            {
                sqlStr = @"insert into metadata_entry(proj_code,batch_key,item_no,department,subcat,state_name,filename,section,series,acc,subscriber_name,birth_date,accept_date,hrms_id,mobile,created_by,created_dttm,status) values('" +
                        projKey + "','" + bundleKey + "','" + itemno +
                        "','" + dept + "','" + subCat + "','" + state_name + "','" + filename + "','" + section + "','" + series + "','" + acc + "','" + subsc_name + "','" + bdate + "','" + adate + "','" + hrms + "','" + mobile + "','" + crd.created_by + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',5)";
                //sqlCmd.Connection = sqlCon;
            }
            sqlCmd.Connection = sqlCon;
        ...
        public bool updateMetaEdit(string dept, string subCat, string state_name, string section, string series, string acc, string subsc, string dob, string adate, string hrms, string mobile)
        {
            bool ret = false;
            if (ret == false)
            {
                _UpdateMetaEdit(projKey, bundleKey, old_filename, filename, dept, subCat, state_name, section, series, acc, subsc, dob, adate, hrms, mobile);
        ...
        public bool _UpdateMetaEdit(string projKey, string bundleKey, string oldFileName, string fileName,
            string dept, string subCat, string state_name, string section, string series, string acc, string subsc, string dob, string adate, string hrms, string mobile)
        {
            string sqlStr = null;
            ...
            sqlStr = "UPDATE metadata_entry SET filename = '" + fileName + "',department  = '" + dept + "',subcat = '" + subCat + "'," +
                ...
                "birth_date='" + dob + "',accept_date ='" + adate + "',hrms_id='"+hrms+"',mobile='"+mobile+"',modified_by ='" + crd.created_by + "',modified_dttm = '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'  WHERE proj_code = '" + projKey + "' AND batch_key = '" + bundleKey + "' and filename = '" + oldFileName + "' ";
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    if (validate() == true)
                    {
                        ...
                        if (checkFileNotExists(filename,dept,subCat) == true)
                        {
                            ...
                            bool insertCase = insertIntoMeta(item, dept, subCat, statename, filename, section, series, acc, subscriber, bDate, aDate, hrms, mobile, sqlTrans);
                            if (insertCase == true)
                            {
                                if (sqlTrans == null)
                                {
                    ...
                    if (validate() == true)
                    {
                        ...
                        if (filename != Files.filename)
                        {
                            if (checkFileNotExistsEdit(filename,dept,subCat) == true)
                            {
                                ...
                                bool updateMeta = updateMetaEdit(dept, subCat, statename, section, series, acc, subscriber, bDate, aDate, hrms, mobile);
        ...
        private void frmNomination_KeyUp(object sender, KeyEventArgs e)
        {
